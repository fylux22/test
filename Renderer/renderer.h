    ...
    while (!done)
    {
        ...
        if (showMenu)
        {
            ...
            if (ImGui::Button("Explorer | " ICON_FA_FOLDER))
                explorer = !explorer;
            ImGui::SameLine();
            if (ImGui::Button("Players | " ICON_FA_PEOPLE_GROUP))
            ...
            switch (category)
            {
                ...
                case 1: // Auto Parry
                {
                    ImGui::Checkbox("Auto Parry Enabled", &Options::AutoParry::Enabled);

                    ImGui::SameLine(400);
                    ImGui::SetNextItemWidth(100);
                    KeyBind::KeyBindButton("Parry Key", &Options::AutoParry::ParryKey);

                    ImGui::Checkbox("Show Status", &Options::AutoParry::ShowStatus);
                    
                    ImGui::NewLine();
                    ImGui::SliderFloat("Range Delay (seconds)", &Options::AutoParry::RangeDelay, 0.0f, 1.0f, "%.2f");
                    ImGui::SliderFloat("Hold Time (seconds)", &Options::AutoParry::HoldTime, 0.1f, 2.0f, "%.2f");
                    ImGui::SliderFloat("Detection Range (studs)", &Options::AutoParry::DetectionRange, 5.0f, 50.0f, "%.1f");
                    ...
                    if (Options::AutoParry::Enabled) {
                        ...
                        if (Options::AutoParry::TargetPlayer.address) {
                            ImGui::Text("Target: %s", Options::AutoParry::TargetPlayer.Name.c_str());
                            
                            auto localCharacter = Globals::Roblox::LocalPlayer.Character();
                            auto localHRP = localCharacter.FindFirstChild("HumanoidRootPart");
                            auto playerHRP = Options::AutoParry::TargetPlayer.HumanoidRootPart;
                            
                            if (localHRP.address && playerHRP.address) {
                                auto distance = (localHRP.Position() - playerHRP.Position()).Magnitude();
                                ImGui::Text("Distance: %.1f studs", distance);
                            }
                        } else {
                            ImGui::Text("Target: None");
                        }
                        
                        if (!Options::AutoParry::LastDetectedAnimation.empty()) {
                            ImGui::Text("Last Animation: %s", Options::AutoParry::LastDetectedAnimation.c_str());
                        }
                        
                        if (Options::AutoParry::IsParrying) {
                            ImGui::TextColored(ImVec4(0, 1, 0, 1), "PARRYING NOW!");
                        }
                    } else {
                    ...
                    ImGui::NewLine();
                    ImGui::TextWrapped("Auto Parry will detect attack animations from nearby players and automatically parry when threats are detected.");
                ...
                case 2: // Misc.
                {
                    ...
                    ImGui::NewLine();
                    ImGui::TextWrapped("Basic camera controls for better visibility.");
                    break;
                }
                case 3:
                {
        ...
        {
            if (!showMenu && IsGameOnTop("Roblox"))
                RunAutoParry(ImGui::GetBackgroundDrawList());
        }
