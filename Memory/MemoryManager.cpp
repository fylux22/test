...
std::string MemoryManager::readString(uintptr_t address) {
	...
	const int MAX_STRING_LENGTH = 4096; // Reasonable limit
	int maxLength = (StrLength > 0 && StrLength < MAX_STRING_LENGTH) ? StrLength : MAX_STRING_LENGTH;

	while (offset < maxLength && (character = read<char>(address + offset)) != 0)
	{
