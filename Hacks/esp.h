...
inline void RenderESP(ImDrawList* drawList)
{
    ...
    std::vector<RobloxPlayer> playerObjectsCopy;
    {
        std::lock_guard<std::mutex> lock(Globals::Caches::PlayerObjectCacheMutex);
        if (Globals::Caches::CachedPlayerObjects.empty())
            return;
        playerObjectsCopy = Globals::Caches::CachedPlayerObjects;
    }

    for (auto& player : playerObjectsCopy)
    {
